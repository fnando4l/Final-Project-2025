...
- `PM25`: Numeric, concentration of fine particulate matter (µg/m³), filtered from `Data Value` where `Name` is "Fine particles (PM 2.5)".
...
- `Time Period`: Time frame of measurement from `Time.Period`.
...
pm25_data <- air_data %>%
  ...
  select(Location = `Geo.Place.Name`, PM25 = `Data.Value`, Start_Date, Time.Period) %>%
...
```{r}
ggplot(pm25_subset, aes(x = Location, y = PM25)) +
...
```{r}
t_test_result <- t.test(PM25 ~ Location, data = pm25_subset)
t_test_result
```
...
```{r}
anova_result <- aov(PM25 ~ Month, data = pm25_subset)
...
Inferential tests addressed two questions: the t-test showed a statistically significant difference in PM2.5 between Upper West Side and Rockaway and Broad Channel (p < 0.05), suggesting localized pollution factors. The ANOVA test for monthly variation indicated no significant seasonal effects (p > 0.05). These findings underscore spatial disparities in NYC air quality and support the need for targeted public health interventions.
